...
* Data Distribution. For the following variables, the distribution is not normal but instead skewed to either the left or the right. In addition, there are also significant outliers which need to be handled by either transformations or exclusions from the dataset prior to modelling. Categorical variables have factor imbalances i.e. more occurence of one factor as compared to the others. 
  + Gross Amount of Loan Approved by Bank (***GrAppv***)
...
For the second research question, Tables 4 through 7 and Figure 4 in the Appendix present the descriptive statistics. The key findings include:
...
* The proportion of whether the loan is approve or not, depends almost equally (46.8% v.s. 53.2%) on whether the business has a revolving line of credit. 
...
* Likewise, the more jobs retained by the company, the less likely the business will default. 
* Almost all of the small business in this analysis are not involved in the LowDoc Loan Program. 
...
1.	The first challenge is the exclusion of loan records values beyond the binary (Y/N) choices that were showing up within the variable. The reason for that was there was lack of information about what the different values represent. And so by applying restrictions to only include Y/N values, there is a potential loss of information. 
2.	The second challenge is that there are variables that are in line with the outcome variable, balance gross, MIS status and charge off amount. These variables have to excluded from the analysis as they are all bi-variate outcomes. 
3.	The data was too large causing inefficiency. 
4.	Due to the data not being as clean as it should have been, it kind of resulted it to becoming deleted. If we had found an easier way rather than removing, the data would have been more consistent. 
